...
def main():
    ...
    for run in range(1):
        ...
        model_nll.load_state_dict(torch.load(os.path.join(save_folder, 'nll_model_08_v2_ratio_{}'.format(args.ratio))))
        ...
        model_task.load_state_dict(torch.load(os.path.join(save_folder, 'task_model_08_v2_ratio_{}'.format(args.ratio))))
        ...
        # nll_mcdp, task_mcdp = nets.eval_net("mcdp", model_nll, variables, params, save_folder)
        
        # nll_mcdp_list.append(nll_mcdp.sum())
        # task_mcdp_list.append(task_mcdp.sum())

        # nll_mcdptask, task_mcdptask = nets.eval_net("mcdp-task", model_task, variables, params, save_folder)
        
        # nll_mcdptask_list.append(nll_mcdptask.sum())
        # task_mcdptask_list.append(task_mcdptask.sum())

    #plot.plot_results(map(lambda x: os.path.join(base_save, str(x)), range(5)), base_save)
    ...
    #nll_mcdp_list = np.array(nll_mcdp_list)
    #task_mcdp_list = np.array(task_mcdp_list)
    
    
    #nll_mcdptask_list = np.array(nll_mcdptask_list)
    #task_mcdptask_list = np.array(task_mcdptask_list)
    ...
    # print('MCDP based neural network NLL: {}, Task: {}'.format(nll_mcdp_list.mean(), task_mcdp_list.mean()))
    # print('MCDP task based neural network NLL: {}, Task: {}'.format(nll_mcdptask_list.mean(), task_mcdptask_list.mean()))
    ...
    # print('MCDP based neural network NLL: {}, Task: {}'.format(nll_mcdp_list.std(), task_mcdp_list.std()))
    # print('MCDP task based neural network NLL: {}, Task: {}'.format(nll_mcdptask_list.std(), task_mcdptask_list.std()))
